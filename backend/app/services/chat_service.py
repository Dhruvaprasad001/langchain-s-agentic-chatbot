...
class ChatService:
    ...
    async def stream_reply(
        ...
        return self._generate_and_persist(uid, session_id, messages, model)
    ...
    async def _generate_and_persist(
        ...
        async for delta in self._call_openai(messages, model, api_key):
            accumulated += delta
    ...
    async def _call_openai(
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"OpenAI returned {exc.response.status_code}",
            ) from exc
    ...
    @staticmethod
    def _build_messages(history: list[Message], user_text: str) -> list[dict]:
